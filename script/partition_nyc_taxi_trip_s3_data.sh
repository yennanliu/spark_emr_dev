...
to_yellow_bucket=s3://nyc-tlc-taxi/yellow_trip
to_green_bucket=s3://nyc-tlc-taxi/green_trip
to_fhv_bucket=s3://nyc-tlc-taxi/fhv_trip
...
for s3_file in $s3_files;
    ...
    if [[ $s3_file =~ ^yellow_tripdata ]] 
       then   
         ...
         aws s3 cp $from_bucket/$s3_file $to_yellow_bucket/dt=$to_file/$s3_file
    
    elif [[ $s3_file =~ ^green_tripdata ]] 
        then  
          to_file=$(echo $s3_file | cut -d " "  -f 4 | cut -d "_" -f 3 | cut -d "." -f 1)
          #echo "copy" $from_bucket/$s3_file "to" $to_green_bucket/dt=$to_file/$s3_file
          aws s3 cp $from_bucket/$s3_file $to_green_bucket/dt=$to_file/$s3_file 

    elif [[ $s3_file =~ ^fhv_tripdata ]] 
        then  
          to_file=$(echo $s3_file | cut -d " "  -f 4 | cut -d "_" -f 3 | cut -d "." -f 1)
          #echo "copy" $from_bucket/$s3_file "to" $to_fhv_bucket/dt=$to_file/$s3_file 
          aws s3 cp $from_bucket/$s3_file $to_fhv_bucket/dt=$to_file/$s3_file
    else 
        continue 
    fi 
    done 
